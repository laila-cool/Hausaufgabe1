...
//	private Projectile [] projectiles = new Projectile[30];
	...
	public void init() {
		...
		frameWork.setBackground(new Background("src\\assets\\space14.jpg"));

		ship = new Ship(screenWidth / 2, 4 * screenHeight / 5, screenWidth / 9, screenWidth / 9,
				"src\\assets\\ship23.png");
		frameWork.addGameObject(ship);

		Ufo ufo = new Ufo(-20, screenHeight / 5, screenWidth / 10, screenWidth / 19, 1, "src\\assets\\ufo20.png");
		ufos.add(ufo);
	...
	public void shoot() {
		...
		Projectile projectile = new Projectile(ship.getX() + ship.getWidth() / 4, ship.getY() - ship.getWidth() / 2,
				ship.getWidth() / 2, ship.getWidth() / 2, 3, "src\\assets\\projectile06.png");
